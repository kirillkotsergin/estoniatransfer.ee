...
export const facts = {
  ...
  legal: {
    ...
    /**
     * ⚠️ ПРОВЕРЬ по самому документу, прежде чем верить этой строке.
     *
     * Орган-эмитент в Эстонии менялся: до 2021 года это был Maanteeamet.
     * `recognizedBy` в разметке — проверяемое утверждение о государственном
     * органе, и неверное здесь хуже, чем отсутствующее: заявить не тот орган
     * значит соврать машиночитаемо. Не сходится с документом — правьте тут,
     * разметка подхватит.
     */
   ...
   * поверхности сразу.
   *
   * Год и «Hybrid» добавлены 23.09.2026 по указанию владельца. До этого здесь
   * стояло просто «Toyota Corolla», а «Hybrid» жил отдельно в alt подписи к
   * фотографиям — то есть сайт описывал одну и ту же машину двумя разными
   * строками. Теперь строка одна.
   *
   * ⚠️ Год ничем, кроме слов владельца, не подтверждён: в репозитории его не
   * было нигде. Это заявление о конкретном автомобиле, и проверить его можно
   * глазами на парковке — если 2020 неверный, правьте здесь, разметка и
   * llms.txt подхватят.
   *
   * ⚠️ НЕ обновляются отсюда и правятся руками: alt подписей к фотографиям
   * (src/components/Car.astro, CarGallery.astro) и ключи "car.title" в ui
   * ниже. Меняете строку — пройдитесь `grep -rn "Corolla" src/`.
   */
