...
export const ui = {
  ru: {
    // 59 знаков. Ключ впереди, бренда нет намеренно: его пока никто не ищет, а
    // место в выдаче стоит отдать словам, которые люди вводят сами.
    // 25.09.2026 «от 130 €» заменено на «цена без доплат»: у конкурента в
    // выдаче «ориентировочная цена», и точная сумма — то, чем мы отличаемся.
    // Сама цифра осталась в description.
    "meta.title": "Трансфер Таллинн — Нарва, Койдула, Лухамаа: цена без доплат",
    // 151 знак. Переписано 25.09.2026 под то же отличие: фиксированная цена и
    // лицензия. Именно «без СКРЫТЫХ доплат», а не «без доплат»: место в
    // очереди (+50 €) — доплата, пусть и по желанию, и голое «без доплат»
    // было бы неправдой в первой же строке сниппета.
    // Направления и цены здесь с 13.09.2026: Google подставлял вместо
    // описания служебную строку формы; от этого же защищает data-nosnippet в
    // OrderForm и Footer. Показ текст не гарантирует — сниппет Google волен
    // переписать.
    "meta.description": `Точная фиксированная цена за машину, без скрытых доплат: Таллинн — Нарва ${facts.routes[0].price} €, Койдула и Лухамаа ${facts.routes[1].price} €. Сумма известна до поездки. Лицензия ${facts.legal.transportLicence}.`,
