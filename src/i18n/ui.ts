...
export const facts = {
   ...
   *
   * - Teenindaja kaart (карта обслуживающего лица). Её выдают человеку, а не
   *   фирме, поэтому в разметке она могла бы висеть только на узле Person —
   *   а Person с одним именем без фамилии сопоставить не с чем и как сигнал
   *   он пустой. Решением владельца 23.09.2026 карту не публикуем вовсе.
   *   Вернётся вместе с полным именем водителя и страницей «О сервисе».
   * - KMKR (VAT). Не заявлен. Появится — отдельным полем сюда и в `vatID`
   *   разметки, а не в `identifier`: для НДС у schema.org своё свойство.
   */
  legal: {
    name: "Barabara OÜ",
