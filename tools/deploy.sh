...
# Снимок ЖИВОЙ карты сайта до отправки. По нему потом видно, что реально
# изменилось, — иначе пришлось бы слать в IndexNow весь сайт, а повторная
# отправка неизменившихся адресов считается спамом и однажды отключит канал.
#
# Ошибку не считаем фатальной: сайт может быть недоступен, и это не повод
# отменять выкладку. changed-urls.mjs увидит нечитаемый файл и молча
# пропустит отправку — лучше не отправить ничего, чем отправить всё.
SITEMAP_BEFORE="$(mktemp)"
trap 'rm -f "$SITEMAP_BEFORE"' EXIT
curl -sS -m 20 "$SITE/sitemap.xml" -o "$SITEMAP_BEFORE" 2>/dev/null || true

echo "▸ отправка"
tar -C dist -cf - . 2>/dev/null | "${SSH[@]}" "cd '$REMOTE' && tar -xmf -"

if checks; then
  # --- IndexNow, только по изменившимся адресам --------------------------
  # После проверок, а не до: отправлять поисковикам страницу, которая на
  # живом сайте ещё не открылась, значит звать их на 404.
  #
  # Google в IndexNow не участвует — ему изменения приходят через lastmod
  # карты сайта. Это про Bing (а значит про ChatGPT Search и Copilot) и
  # про Яндекс (а значит про Алису).
  echo "▸ IndexNow"
